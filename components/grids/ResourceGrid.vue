...
export default {
  ...
  methods: {
    ...
    resourceFilters(resource) {
      ...
      if (this.exclude && this.author.length) {
        if (Array.isArray(this.author)) {
          return (
            resource.author.some((cat) => {
              return this.author.includes(cat)
            }) && resource.slug !== this.exclude
          )
        }
        return (
          resource.author.includes(this.author) &&
          resource.slug !== this.exclude
        )
      }
      if (this.author.length) {
        if (Array.isArray(this.author)) {
          return resource.author.some((cat) => {
            return this.author.includes(cat)
          })
        }
